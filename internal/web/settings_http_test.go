...
func TestAdminSettingsHTTP(t *testing.T) {
	...
	var getBody struct {
		...
		DefaultModelMappings []modelMapping  `json:"defaultModelMappings"`
	}
	if err := json.Unmarshal(w.Body.Bytes(), &getBody); err != nil {
		t.Fatal(err)
	}
	if len(getBody.Settings.ModelMappings) == 0 || len(getBody.CodexModels) == 0 || len(getBody.UpstreamTones) == 0 || len(getBody.DefaultModelMappings) == 0 {
